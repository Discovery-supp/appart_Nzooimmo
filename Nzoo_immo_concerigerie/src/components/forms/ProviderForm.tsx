        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
              ...
              <button
                type="button"
                onClick={currentStep === steps.length - 1 ? () => console.log('Submit provider form', { formData, personalInfo, professionalInfo, serviceInfo, pricingInfo }) : nextStep}
                className="px-8 py-3 bg-primary text-white font-semibold rounded-lg hover:bg-primary-light transition-all shadow-lg"
              >
                {currentStep === steps.length - 1 ? 'Soumettre ma candidature' : 'Suivant'}
              </button>
            </div>
          </form>
        </div>
